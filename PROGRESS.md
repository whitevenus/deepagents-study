...
## 📍 Phase 6 · 自我进化(反思提炼)进行中
目标(终止条件):故意会失败的任务,第二轮因吸取教训而成功。

### 轮 16 · 反思提炼:复盘裁判 + 教训沉淀(Reflexion-lite)✅ (2026-06-25)
- **决策**(用户定):闭环做「**相似任务下次更好**」(不改 worker 重试逻辑)+ **轻量自评 critic**(单 LLM 裁判)。人工反馈评判 = 企业级阶段必做,本轮延后(见 backlog)。
- **核心墙**:演示「第二轮成功」必须有「成功/失败」信号,否则反思无从知道错在哪 = 项目反复撞的 **verifier 瓶颈**。无沙箱/无人工时,用 LLM 当裁判是最现实的失败信号(代价:裁判本身不完美)。
- **建了两块**:
  - `executor.reflect(title, desc, result, status)`:单次 LLM 调用同出 `{adequate, lesson}`——裁判结果是否达标 + 提炼 ≤2 句可复用教训(不达标/失败说清问题+下次怎么做,达标提炼关键点)。
  - `worker._remember` 升级:不再存**原文整段**,而是 `reflect` 提炼的**教训**(库更精,避免长报告污染上下文);done 和 **failed 都复盘**(失败也学:为何失败/下次避免)。教训靠轮 15 的预检索注入送回下一轮。
- **顺手修**:失败信息为空 bug(`str(TimeoutError())` 是空串 → 兜底用异常类名)。
- **verify**:
  - ✅ pytest 37 passed(_remember 在 SQLite 下 is_pg 守卫前置 no-op,不触达 LLM,测试不受影响)。
  - ✅ `reflect_demo`(真 PG+API):正确结果判达标;漏世纪年规则的闰年代码判**不达标**,给出精准纠正教训(「能被 400 整除,或能被 4 整除但不能被 100 整除」);教训入库后相似查询 0.72 命中。
  - 闭环每一环有实证:reflect 裁判+提炼 ✅(本轮)/ 教训入库可检索 ✅(本轮)/ 注入送达模型改变输出 ✅(轮 15 ZorbaX)。
- **关于终止条件**:机制全部就绪并实证。「同一个会失败的任务真·第二轮成功」的全自动 live 演示依赖「让第一轮可靠地失败」(对强模型而言要刻意构造,且裁判是 LLM 不完美)——这是 LLM 行为 eval,本项目一贯不设门禁;真严格闭环需更强 verifier(人工反馈/多视角投票/沙箱跑测试),留企业级阶段。
- **存储闸(轮 16 收尾补)**:无差别复盘会把「答得好好的常规任务」的正确废话教训也存进库 → 污染检索/注入。修法:`reflect` 加 `reusable` 字段(prompt 要求吝啬:只有失败/不达标/非显然洞见才 true),`_remember` 只在 `reusable` 时才 `add_knowledge`。reflect 调用很便宜(trace 实测 209-779 token,占单任务个位数%),真问题是存储污染,故只加存储闸不加粗筛(那点钱不值得上脆弱启发式)。verify:demo 里「1+1=2」reusable=False 不入库、漏世纪年的闰年代码 reusable=True 入库。
...
- ~~**失败信息为空 bug**~~ ✅ 已修(轮 16):`str(last_err) or type(last_err).__name__`,TimeoutError 空串时兜底显示类名。
